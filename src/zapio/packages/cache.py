"""Cache management for Zapio packages.
...
class Cache:
    """Manages the Zapio cache directory structure.

    The cache can be located in the project directory (.zap/) or in a
    global location specified by the ZAPIO_CACHE_DIR environment variable.
    ...
    def __init__(self, project_dir: Optional[Path] = None):
        ...
        cache_env = os.environ.get("ZAPIO_CACHE_DIR")
        if cache_env:
            self.cache_root = Path(cache_env).resolve()
        else:
            self.cache_root = self.project_dir / ".zap" / "cache"

        self.build_root = self.project_dir / ".zap" / "build"
