...
class PlatformIORegistry:
    ...
    def resolve_version(
        self, owner: str, name: str, version_spec: Optional[str] = None
    ) -> LibraryVersion:
        ...
        if not files:
            raise RegistryError(
                f"No download files available for {owner}/{name}@{version_str}"
            )
    ...
    def download_library(
        self, spec: LibrarySpec, dest_dir: Path, show_progress: bool = True
    ) -> Path:
