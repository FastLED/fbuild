...
class CompilationJobQueue:
    ...
    def start(self) -> None:
        ...
        for i in range(self.num_workers):
            worker = Thread(target=self._worker_loop, name=f"CompilationWorker-{i}", daemon=True)
            worker.start()
    ...
    def submit_job(self, job: CompilationJob) -> str:
        ...
        logging.debug(f"Submitted job {job.job_id}: {job.source_path.name}")
        return job.job_id
        ...
        try:
            ...
            with self.jobs_lock:
                ...
                if result.returncode == 0:
                    ...
                    logging.debug(f"Job {job.job_id} completed in {job.duration():.2f}s")
                else:
            ...
            with self.jobs_lock:
                ...
                job.end_time = time.time()
            logging.error(f"Job {job.job_id} timed out: {job.source_path.name}")
    ...
    def wait_for_completion(self, job_ids: list[str], timeout: Optional[float] = None) -> bool:
        ...
        while True:
            ...
            if timeout and (time.time() - start_time) > timeout:
                logging.warning(f"wait_for_completion timed out after {timeout}s")
                return False
    ...
    def clear_jobs(self) -> None:
        ...
        with self.jobs_lock:
            ...
            if to_remove:
                logging.debug(f"Cleared {len(to_remove)} completed jobs")
