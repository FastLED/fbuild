...
def purge_packages(target: Optional[str], dry_run: bool, project_dir: Path) -> bool:
    ...
    Args:
        target: Target to purge ('all', 'project', environment name, or None to list)
        dry_run: If True, show what would be deleted without deleting
        project_dir: Project directory (for project/environment-specific purge)

    Returns:
        True if successful, False otherwise
    """
    # Handle project purge early — doesn't need global cache discovery
    if target == "project":
        return _purge_project(project_dir, dry_run)
...
def _purge_project(project_dir: Path, dry_run: bool) -> bool:
    """Purge all fbuild build artifacts from a project's .fbuild/ directory.

    This removes compiled objects, archives, build state, and all other
    artifacts generated by fbuild. The global cache (toolchains, frameworks,
    platforms) is not affected.

    Args:
        project_dir: Project directory containing .fbuild/
        dry_run: If True, only show what would be deleted
    ...
    fbuild_dir = get_project_fbuild_dir(project_dir)

    if not fbuild_dir.exists():
        log(f"No .fbuild directory found in {project_dir}")
        return True
    ...
    file_count = 0
    try:
        for dirpath, _, filenames in os.walk(fbuild_dir):
            for filename in filenames:
                filepath = Path(dirpath) / filename
                try:
                    total_size += filepath.stat().st_size
    ...
    size_str = PackageDiscovery.format_size(total_size)

    if dry_run:
        log(f"Would delete: {fbuild_dir} ({size_str}, {file_count} files)")
        return True

    log(f"Purging project artifacts: {fbuild_dir} ({size_str})")

    try:
        shutil.rmtree(fbuild_dir)
        log_success(f"Purged project artifacts, freed {size_str}")
        return True
    except (PermissionError, OSError) as e:
        log_error(f"Failed to delete {fbuild_dir}: {e}")
        return False
