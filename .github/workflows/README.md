...
- The full and nightly board matrices execute identical build inputs once.
  Each matrix row carries `workflow_aliases` so every listed board workflow
  remains represented, including legacy spelling aliases.
- Per-board `build-*.yml` files are manual/reusable only. Their triggers are
  generated by `ci/render_workflows.py`; the nightly sweep still runs every
  board independently of PR labels.

## Independent lightweight checks (push/PR)

- **`check-{ubuntu,windows}.yml`** -- Reusable Clippy + tests per platform (no macOS runner: macOS
...
Per-board files and the CI tier workflows get the block from `ci/render_workflows.py`; everything else
...
## Per-Board Builds (manual/reusable, called by CI tiers)
...
`release-auto.yml` is dispatched explicitly with a required exact candidate SHA;
