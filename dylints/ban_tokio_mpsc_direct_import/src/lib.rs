...
dylint_linting::declare_late_lint! {
    /// ### What it does
    ///
    /// Bans `use tokio::sync::mpsc::*` (and any subpath thereof) in
    /// production code, except inside the bridge module
    /// (`crates/fbuild-core/src/channel.rs`).
    ///
    /// ### Why is this bad?
    ///
    /// All async channels in fbuild flow through
    /// `fbuild_core::channel::{bounded, unbounded}` so the workspace
    /// has one source of truth for the channel surface. Direct
    /// imports bypass the bridge and the cohesive surface starts to
    /// fragment.
    ///
    /// FastLED/fbuild#844 (bridge sweep, "Bridge pair 4").
    ///
    /// ### Example
    ///
    /// ```rust,ignore
    /// use tokio::sync::mpsc;                          // banned
    /// use tokio::sync::mpsc::{channel, Sender};       // banned
    /// ```
    ///
    /// Use instead:
    ///
    /// ```rust,ignore
    /// use fbuild_core::channel::{bounded, Sender};
    /// ```
    pub BAN_TOKIO_MPSC_DIRECT_IMPORT,
    Deny,
    "ban direct `use tokio::sync::mpsc` imports outside the fbuild channel bridge"
}

const BRIDGE_MODULES: &[&str] = &["crates/fbuild-core/src/channel.rs"];
...
impl<'tcx> LateLintPass<'tcx> for BanTokioMpscDirectImport {
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx Item<'tcx>) {
        ...
        if let ItemKind::Use(path, _) = &item.kind {
            let segs = path
                ...
                .collect::<Vec<_>>();
            // `use tokio::sync::mpsc[::...]`
            if segs.len() >= 3 && segs[0] == "tokio" && segs[1] == "sync" && segs[2] == "mpsc" {
    ...
    cx.opt_span_lint(
        ...
        DiagDecorator(|diag| {
            diag.primary_message(
                "direct `use tokio::sync::mpsc` bypasses the fbuild channel \
                 bridge. Import from `fbuild_core::channel` instead so the \
                 workspace has one source of truth for the async channel \
                 surface. See FastLED/fbuild#844.",
            );
        }),
    );
}
...
mod tests {
    ...
    fn bridge_module_recognized() {
        assert!(is_bridge_module("crates/fbuild-core/src/channel.rs"));
