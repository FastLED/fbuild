...
def test_psram_detection_xiao_esp32s3():
    ...
    assert config.has_psram is False, f"Board {board_name} incorrectly reports has_psram=True. This board variant has no PSRAM and should report False."
    ...
    assert "CONFIG_ESP32S3_DATA_CACHE_64KB" in build_flags_str, f"Missing cache config flag for no-PSRAM board. Expected CONFIG_ESP32S3_DATA_CACHE_64KB in build flags. Got: {build_flags_str}"

    # Verify PSRAM flags are NOT present
    assert "BOARD_HAS_PSRAM" not in build_flags_str, f"BOARD_HAS_PSRAM should not be set for no-PSRAM variant. Got: {build_flags_str}"
...
def test_build_flags_respect_no_psram():
    ...
    assert has_psram is False, f"board_has_psram() returned {has_psram} for {board_name}. Expected False (this board has no PSRAM)."
    ...
    assert "CONFIG_ESP32S3_DATA_CACHE_64KB" in flags_str or "-DCONFIG_ESP32S3_DATA_CACHE_64KB" in flags_str, (
        f"Missing cache config flag for no-PSRAM board. Expected CONFIG_ESP32S3_DATA_CACHE_64KB in build flags. Got: {flags_str}"
    )

    # Verify PSRAM flags are NOT present
    assert "BOARD_HAS_PSRAM" not in flags_str, f"BOARD_HAS_PSRAM should not be set for no-PSRAM variant. Got: {flags_str}"

    assert "CONFIG_SPIRAM_USE_MALLOC" not in flags_str, f"CONFIG_SPIRAM_USE_MALLOC should not be set for no-PSRAM variant. Got: {flags_str}"
...
def test_psram_board_list_includes_xiao():
    ...
    board_name = "seeed_xiao_esp32s3"
    assert board_name in no_psram_boards, f"Board '{board_name}' not found in NO_PSRAM_BOARDS list. This board has no PSRAM and must be included. Current list: {no_psram_boards}"
...
def test_psram_crash_prevention():
    ...
    assert not has_dangerous_flag, f"DANGEROUS: BOARD_HAS_PSRAM is set for a board without PSRAM! This will cause 'CORRUPT HEAP' crash on boot. Build flags: {flags_str}"

    assert not has_psram_malloc, f"DANGEROUS: CONFIG_SPIRAM_USE_MALLOC is set for a board without PSRAM! This will cause heap allocation failures. Build flags: {flags_str}"
    ...
    assert has_safe_cache_config, f"Missing safe cache configuration for no-PSRAM board. Should have CONFIG_ESP32S3_DATA_CACHE_64KB. Build flags: {flags_str}"
