...
from fbuild.build.linker import LinkerAVR, LinkerError, LinkResult, SizeInfo
...
class TestSizeInfo:
    ...
    def test_size_info_creation(self):
        ...
        size = SizeInfo(text=1000, data=50, bss=100, total_flash=1050, total_ram=150, max_flash=32256, max_ram=2048)
    ...
    def test_flash_percent(self):
        ...
        size = SizeInfo(text=16128, data=0, bss=0, total_flash=16128, total_ram=0, max_flash=32256, max_ram=2048)

        assert size.flash_percent == pytest.approx(50.0, rel=0.01)

    def test_ram_percent(self):
        """Test RAM percentage calculation."""
        size = SizeInfo(text=0, data=512, bss=512, total_flash=0, total_ram=1024, max_flash=32256, max_ram=2048)
    ...
    def test_percent_without_max(self):
        ...
        size = SizeInfo(text=1000, data=50, bss=100, total_flash=1050, total_ram=150)
...
class TestLinker:
    ...
    def mock_tools(self, tmp_path):
        ...
        return {"gcc": avr_gcc, "ar": avr_ar, "objcopy": avr_objcopy, "size": avr_size}

    @pytest.fixture
    def linker(self, mock_tools):
        """Create Linker instance."""
        return LinkerAVR(avr_gcc=mock_tools["gcc"], avr_ar=mock_tools["ar"], avr_objcopy=mock_tools["objcopy"], avr_size=mock_tools["size"], mcu="atmega328p", max_flash=32256, max_ram=2048)

    def test_init_success(self, mock_tools):
        """Test successful linker initialization."""
        linker = LinkerAVR(avr_gcc=mock_tools["gcc"], avr_ar=mock_tools["ar"], avr_objcopy=mock_tools["objcopy"], avr_size=mock_tools["size"], mcu="atmega328p", max_flash=32256, max_ram=2048)
    ...
    def test_init_gcc_not_found(self, mock_tools, tmp_path):
        ...
        with pytest.raises(LinkerError, match="avr-gcc not found"):
            LinkerAVR(avr_gcc=fake_gcc, avr_ar=mock_tools["ar"], avr_objcopy=mock_tools["objcopy"], avr_size=mock_tools["size"], mcu="atmega328p")
    ...
    def test_init_ar_not_found(self, mock_tools, tmp_path):
        ...
        with pytest.raises(LinkerError, match="avr-ar not found"):
            LinkerAVR(avr_gcc=mock_tools["gcc"], avr_ar=fake_ar, avr_objcopy=mock_tools["objcopy"], avr_size=mock_tools["size"], mcu="atmega328p")
    ...
    def test_init_objcopy_not_found(self, mock_tools, tmp_path):
        ...
        with pytest.raises(LinkerError, match="avr-objcopy not found"):
            LinkerAVR(avr_gcc=mock_tools["gcc"], avr_ar=mock_tools["ar"], avr_objcopy=fake_objcopy, avr_size=mock_tools["size"], mcu="atmega328p")
    ...
    def test_init_size_not_found(self, mock_tools, tmp_path):
        ...
        with pytest.raises(LinkerError, match="avr-size not found"):
            LinkerAVR(avr_gcc=mock_tools["gcc"], avr_ar=mock_tools["ar"], avr_objcopy=mock_tools["objcopy"], avr_size=fake_size, mcu="atmega328p")
...
"""
    ...
    def test_link_full_success(self, mock_run, linker, tmp_path):
        ...
        result = linker.link(sketch_objects, core_objects, output_elf, output_hex)
    ...
    def test_link_result_dataclass(self):
        ...
        size_info = SizeInfo(text=1000, data=50, bss=100, total_flash=1050, total_ram=150)

        result = LinkResult(success=True, elf_path=Path("firmware.elf"), hex_path=Path("firmware.hex"), size_info=size_info, stdout="output", stderr="")
