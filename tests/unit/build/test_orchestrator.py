...
from fbuild.build.orchestrator_avr import BuildOrchestratorAVR, BuildOrchestratorError
...
def mock_platformio_config():
    ...
    config.get_env_config = Mock(return_value={"platform": "atmelavr", "board": "uno", "framework": "arduino"})
    return config
...
def mock_source_collection():
    ...
    return SourceCollection(sketch_sources=[Path("src/main.cpp")], core_sources=[Path("core/wiring.c"), Path("core/main.cpp")], variant_sources=[Path("variant/pins.c")], headers=[])
...
def mock_linker():
    ...
    size_info = SizeInfo(text=1000, data=50, bss=100, total_flash=1050, total_ram=150, max_flash=32256, max_ram=2048)

    link_result = LinkResult(success=True, elf_path=Path("build/firmware.elf"), hex_path=Path("build/firmware.hex"), size_info=size_info, stdout="", stderr="")
...
class TestLoadBoardConfig:
    ...
    def test_load_board_config_uno(self):
        ...
        orchestrator = BuildOrchestratorAVR()
        env_config = {"platform": "atmelavr", "board": "uno", "framework": "arduino"}
    ...
    def test_load_board_config_with_overrides(self):
        ...
        orchestrator = BuildOrchestratorAVR()
        env_config = {"platform": "atmelavr", "board": "uno", "framework": "arduino", "board_build.f_cpu": "8000000L"}
    ...
    def test_load_board_config_unknown(self):
        ...
        orchestrator = BuildOrchestratorAVR()
        env_config = {"platform": "atmelavr", "board": "unknown_board", "framework": "arduino"}
...
class TestScanSources:
    ...
    def test_scan_sources(self, mock_scanner_class, tmp_path, mock_board_config):
        """Test source scanning."""
        mock_scanner = Mock()
        mock_collection = SourceCollection(sketch_sources=[Path("src/main.cpp")], core_sources=[Path("core/wiring.c")], variant_sources=[Path("variant/pins.c")], headers=[])
        ...
        sources = orchestrator._scan_sources(tmp_path, tmp_path / "build", mock_board_config, core_path)
    ...
    def test_create_compiler(self, mock_toolchain, mock_board_config):
        ...
        with patch("fbuild.build.orchestrator.Compiler") as mock_compiler_class:
            ...
            _compiler = orchestrator._create_compiler(  # type: ignore[attr-defined]
                mock_toolchain, mock_board_config, core_path
            )
    ...
    def test_create_linker(self, mock_toolchain, mock_board_config):
        ...
        with patch("fbuild.build.orchestrator.Linker") as mock_linker_class:
            ...
            _linker = orchestrator._create_linker(  # type: ignore[attr-defined]
                mock_toolchain, mock_board_config
            )
...
class TestCompileSources:
    ...
    def test_compile_sources_success(self, tmp_path, mock_compiler):
        ...
        objects = orchestrator._compile_sources(  # type: ignore[attr-defined]
            mock_compiler, sources, output_dir, verbose=False, source_type="sketch"
        )
...
class TestBuildFullProcess:
    ...
    def test_build_success(self, mock_linker_class, mock_compiler_class, mock_scanner_class, mock_core_class, mock_toolchain_class, mock_config_class, tmp_path):
        ...
        mock_config.get_env_config = Mock(return_value={"platform": "atmelavr", "board": "uno", "framework": "arduino"})
        mock_config_class.return_value = mock_config

        # Toolchain
        mock_toolchain = Mock()
        mock_toolchain.ensure_toolchain = Mock()
        mock_toolchain.get_all_tools = Mock(
            return_value={
        ...
        # Source scanner
        mock_scanner = Mock()
        mock_sources = SourceCollection(sketch_sources=[Path("src/main.cpp")], core_sources=[Path("core/wiring.c")], variant_sources=[], headers=[])
