...
from fbuild.build.compiler_avr import CompilerAVR, CompilerError, CompileResult
...
class TestCompiler:
    ...
    def defines(self):
        ...
        return {"ARDUINO": "10819", "ARDUINO_AVR_UNO": "", "ARDUINO_ARCH_AVR": ""}

    @pytest.fixture
    def compiler(self, mock_gcc, mock_gpp, includes, defines):
        """Create Compiler instance."""
        return CompilerAVR(avr_gcc=mock_gcc, avr_gpp=mock_gpp, mcu="atmega328p", f_cpu="16000000L", includes=includes, defines=defines)

    def test_init_success(self, mock_gcc, mock_gpp, includes, defines):
        """Test successful compiler initialization."""
        compiler = CompilerAVR(avr_gcc=mock_gcc, avr_gpp=mock_gpp, mcu="atmega328p", f_cpu="16000000L", includes=includes, defines=defines)
    ...
    def test_init_gcc_not_found(self, tmp_path, mock_gpp, includes, defines):
        ...
        with pytest.raises(CompilerError, match="avr-gcc not found"):
            CompilerAVR(avr_gcc=fake_gcc, avr_gpp=mock_gpp, mcu="atmega328p", f_cpu="16000000L", includes=includes, defines=defines)
    ...
    def test_init_gpp_not_found(self, mock_gcc, tmp_path, includes, defines):
        ...
        with pytest.raises(CompilerError, match="avr-g\\+\\+ not found"):
            CompilerAVR(avr_gcc=mock_gcc, avr_gpp=fake_gpp, mcu="atmega328p", f_cpu="16000000L", includes=includes, defines=defines)
    ...
    def test_compile_sources_success(self, mock_run, compiler, tmp_path):
        ...
        sources = [tmp_path / "test1.c", tmp_path / "test2.cpp", tmp_path / "test3.c"]
    ...
    def test_compile_sources_failure(self, mock_run, compiler, tmp_path):
        ...
        sources = [tmp_path / "test1.c", tmp_path / "test2.c"]
    ...
    def test_compile_result_dataclass(self):
        ...
        result = CompileResult(success=True, object_file=Path("test.o"), stdout="output", stderr="", returncode=0)
