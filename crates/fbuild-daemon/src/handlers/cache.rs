...
pub async fn cache_stats(State(_ctx): State<Arc<DaemonContext>>) -> Json<CacheStatsResponse> {
    let result = tokio::task::spawn_blocking(|| {
        ...
        dc.stats()
            .map_err(|e| format!("failed to read cache stats: {}", e))
    })
    .await;

    match result {
        Ok(Ok(stats)) => Json(CacheStatsResponse {
            success: true,
        ...
        Ok(Err(msg)) => Json(CacheStatsResponse {
            success: false,
            message: Some(msg),
            ..Default::default()
        }),
        Err(e) => Json(CacheStatsResponse {
            success: false,
            message: Some(format!("blocking task failed: {}", e)),
            ..Default::default()
        }),
    }
}
...
pub async fn run_gc(State(ctx): State<Arc<DaemonContext>>) -> Json<GcResponse> {
    ...
    let result = tokio::task::spawn_blocking(|| {
        let dc = fbuild_packages::DiskCache::open()
            .map_err(|e| format!("failed to open cache: {}", e))?;
        dc.run_gc().map_err(|e| format!("GC failed: {}", e))
    })
    .await;

    match result {
        Ok(Ok(report)) => Json(GcResponse {
            success: true,
