...
//! [`crate::ini_parser::PlatformIOConfig::get_lib_deps`] and figures out
//! what kind of dependency source it is (registry, GitHub, generic Git,
//! HTTP archive, symlink, `file://`, or local path). Local sources map to
...
//!
//! Originally lived in `fbuild-cli::sync::source` (FastLED/fbuild#618); moved
//! here (FastLED/fbuild#1076 Phase 2) so `fbuild-daemon`'s read-only
//! `/api/ide/libraries` endpoint can reuse the exact same classification
//! logic as `fbuild sync` without `fbuild-daemon` depending on `fbuild-cli`
//! (disallowed — `fbuild-cli` is a thin HTTP client of the daemon, not the
//! other way around). `fbuild-cli`'s `sync::source` module now just
//! re-exports this.
