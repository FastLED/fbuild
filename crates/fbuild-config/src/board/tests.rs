...
/// `variants/<board>/pins_arduino.h` already defines USB_VID/USB_PID, so
/// injecting them again via `-D` produces a "USB_VID redefined" warning at
/// every TU that includes pins_arduino.h (149-156 per build observed in CI).
/// The fix: get_defines() must NOT emit USB_VID/USB_PID for ESP32-S2/S3
/// boards even when the board JSON carries vid/pid fields.
#[test]
fn test_esp32s3_board_skips_usb_vid_pid_injection() {
    ...
    // The board JSON has vid/pid set (Adafruit Feather ESP32-S3 = 0x239A/0x811B).
    assert!(
        config.vid.is_some(),
    ...
    // But get_defines() must NOT emit them because the framework variant
    // header already defines them.
    let defines = config.get_defines();
    assert!(
...
/// User override path: when a user sets `-DUSB_VID=...` via build_flags, the
/// extra_flags processing should still propagate it. The skip rule only
/// applies to the unconditional injection from the JSON `vid`/`pid` fields.
#[test]
fn test_esp32s3_board_extra_flag_usb_vid_override_wins() {
    ...
    // Simulate user override via build_flags / board.extra_flags.
