//! Fetch primitives + package base traits for fbuild (FastLED/fbuild#1008 B).
//!
//! The base layer of the `fbuild-packages` split: URL-based download + archive
//! extraction + cache/disk-cache, plus the `Package` / `Toolchain` / `Framework`
//! traits and `PackageBase` that the `fbuild-library` and `fbuild-toolchain`
//! crates build on. Re-exported (with those two) by the `fbuild-packages` facade.
