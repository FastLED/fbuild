...
- **`daemon_crash_recovery.rs`** -- regression for FastLED/fbuild#1228 and
  #1320.
  `#[ignore]`-gated (runs under `bash test --full`): spawns the real
  `fbuild` + sibling `fbuild-daemon` binaries, kills the daemon uncleanly,
  and asserts both daemon management and `clean sketch` respawn a fresh
  daemon instead of redialing the dead endpoint. Skips when a live dev daemon
  owns the real `~/.fbuild/dev` root or no sibling daemon binary exists.
