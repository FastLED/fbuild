//! Silicon Labs Arduino core framework package.
//!
//! Downloads and manages the SiliconLabs Arduino core from GitHub.
//! Provides paths to: cores/silabs, variants/, libraries/.
...
impl SilabsCores {
    ...
    fn validate(install_dir: &Path) -> fbuild_core::Result<()> {
        ...
        let arduino_h = root.join("cores/silabs/Arduino.h");
        if !arduino_h.exists() {
            return Err(fbuild_core::FbuildError::PackageError(format!(
                "Silicon Labs core missing cores/silabs/Arduino.h (in {})",
                root.display()
            )));
        }
...
impl crate::Package for SilabsCores {
    ...
    fn is_installed(&self) -> bool {
        ...
        root.join("cores").join("silabs").join("Arduino.h").exists()
    }
...
impl Framework for SilabsCores {
...
///
/// GitHub archives extract as `arduino-2.2.0/` with the core inside.
...
mod tests {
    ...
    fn test_silabs_cores_not_installed() {
        ...
        let core = SilabsCores::with_cache_root(tmp.path(), &tmp.path().join("cache"));
    ...
    fn test_find_core_root_direct() {
        let tmp = tempfile::TempDir::new().unwrap();
        std::fs::create_dir_all(tmp.path().join("cores/silabs")).unwrap();
    ...
    fn test_find_core_root_nested() {
        let tmp = tempfile::TempDir::new().unwrap();
        let nested = tmp.path().join("arduino-2.2.0");
        std::fs::create_dir_all(nested.join("cores/silabs")).unwrap();
    ...
    fn test_get_linker_script() {
        let tmp = tempfile::TempDir::new().unwrap();
        let core = SilabsCores::new(tmp.path());
        let script = core.get_linker_script("xg24explorerkit");
        assert!(script.to_string_lossy().contains("linker_script.ld"));
    }
    ...
    fn test_validate_missing_arduino_h() {
        let tmp = tempfile::TempDir::new().unwrap();
        let result = SilabsCores::validate(tmp.path());
