...
impl Rp2040Cores {
    ...
    fn validate(install_dir: &Path) -> fbuild_core::Result<()> {
        ...
        let arduino_h = root.join("cores/rp2040/Arduino.h");
        if !arduino_h.exists() {
...
impl Framework for Rp2040Cores {
...
///
/// GitHub archives extract as `arduino-pico-4.5.3/` with the core inside.
...
mod tests {
    ...
    #[test]
    fn test_rp2040_cores_not_installed() {
    ...
    fn test_find_core_root_direct() {
        let tmp = tempfile::TempDir::new().unwrap();
        std::fs::create_dir_all(tmp.path().join("cores/rp2040")).unwrap();
    ...
    fn test_find_core_root_nested() {
        let tmp = tempfile::TempDir::new().unwrap();
        let nested = tmp.path().join("arduino-pico-4.5.3");
        std::fs::create_dir_all(nested.join("cores/rp2040")).unwrap();
    ...
    fn test_validate_missing_arduino_h() {
        let tmp = tempfile::TempDir::new().unwrap();
        let result = Rp2040Cores::validate(tmp.path());
