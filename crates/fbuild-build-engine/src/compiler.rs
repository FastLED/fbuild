...
// Rebuild-signature fingerprints live in `rebuild_signature.rs` (extracted
// to stay under the 1000-LOC limit); re-exported here so the public
// `crate::compiler::*` paths — used by the platform crates and this
// module's own write side — are unchanged.
pub use crate::rebuild_signature::{
    build_rebuild_signature, build_rebuild_signature_for_project,
