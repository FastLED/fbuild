...
impl EspQemu {
    ...
    pub async fn resolve_executable(&self) -> Result<PathBuf> {
        ...
        // apt-get anything.
        ensure_qemu_can_start(&resolved, &self.project_dir).await?;
        Ok(resolved)
    }
...
fn probe_qemu_binary(qemu_binary: &Path, lib_dir: Option<&Path>) -> QemuProbe {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let ld_library_path = lib_dir.map(|dir| {
            crate::toolchain::esp_qemu_runtime::ld_library_path_with(
                dir,
...
async fn ensure_qemu_can_start(qemu_binary: &Path, project_dir: &Path) -> Result<()> {
    ...
    let runtime = crate::toolchain::esp_qemu_runtime::QemuLinuxRuntime::new(project_dir)
