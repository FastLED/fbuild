...
///
/// `pub` (rather than the usual private `#[pymodule]` fn) so the embedded-
/// CPython integration tests in `tests/python_facades.rs` can register it
/// via `pyo3::append_to_inittab!` before starting the interpreter.
#[pymodule]
